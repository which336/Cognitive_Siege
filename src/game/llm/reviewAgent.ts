...
function sanitize(parsed: any, nextWaveIndex: number): NextStrategy {
  ...
  const allowedKinds = getAllowedEnemyKindsForWave(nextWaveIndex);
  const allowedSkills = getAllowedSkillsForWave(nextWaveIndex);
  ...
  aggression = clampAggressionForWave(nextWaveIndex, Math.max(-1, Math.min(1, aggression)));
  return {
...
export async function runReviewAgent(opts: RunReviewOpts): Promise<ReviewResult> {
  ...
  try {
    ...
    return {
      ...
      next_strategy: sanitize(parsed, nextWaveIndex),
      fromLLM: true,
    };
  } catch (err) {
