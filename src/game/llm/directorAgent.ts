...
const DIRECTOR_SYSTEM = `你是一个游戏剧情导演。你正在为同一位女性失眠者「林晚」写梦境章节中的波次开场，不是每个章节或每波换一个患者。请基于林晚本轮梦境的情绪标签，生成一段 80-120 字中文情境文案：第三人称、文学化、有具体生活细节（一杯凉透的茶、未读消息、走廊声响等），不要直接谈论游戏机制，不要用第二人称。

连续性要求：
- patientName 必须固定为 "林晚"。
- 林晚是女性，中文代词必须使用“她 / 她的 / 她会”等女性指代。
- 禁止使用“他 / 他的 / 他会”等男性代词指代林晚。
- 所有章节和波次都属于林晚同一个人的故事，可以延续前后情绪，但不要创造新的主角、患者、姓名或职业。
- 可以写“她”“林晚”，不要写成另一个人的经历。

最后返回 JSON：
{
  "patientName": string,
  "wave": number,     // 当前章节内的波次编号
...
export interface DirectorOpts {
  ...
  wave: number;
  emotionHint: string; // 例如“焦虑·临界”或“抑郁·复发”。
  levelId?: string;
  levelName?: string;
  signal?: AbortSignal;
}
...
export async function runDirector(opts: DirectorOpts): Promise<VignetteContext> {
  const { settings, wave, emotionHint, levelId = 'level_1', levelName, signal } = opts;
  if (settings.demoMode || !settings.apiKey) {
    return fallbackVignette(wave, levelId);
  }
  try {
    const res = await chatCompletion(settings, {
      messages: [
        { role: 'system', content: DIRECTOR_SYSTEM },
        { role: 'user', content: `章节：${levelName ?? levelId}。第 ${wave} 波，情绪标签：${emotionHint}。请返回 JSON。` },
      ],
      responseFormat: 'json',
      temperature: 0.9,
      maxTokens: 300,
    ...
    return normalizeVignette({
      patientName: '林晚',
      wave: typeof parsed.wave === 'number' ? parsed.wave : wave,
