...
玩家扮演**认知工程师**，进入一名失眠者「林晚」的潜意识，在 6 个梦境章节中放置 6 种「念头塔」抵御 5 种**人格化心魔**的入侵。每个章节是一关，每关包含 10 波战斗；每波结束，由 LLM 扮演的"刚战败的心魔集体意识"会**真实复盘**这一波，并在下一波**真的按这个新思路调整路径、编队、技能与阵容**。
...
每个完整循环 = 1 波战斗，约 90~120 秒。当前版本共 6 关 / 60 波：第 1 关是基础教学，后 5 关分别引入独立地图、路线节奏和强机制，讲述林晚潜意识防线在多个梦境章节中的连续变化。
...
- 焦虑 Boss：展开焦虑光环，全场心魔 +10% 移速；若谈判进入 `confront` 狂暴状态，额外使全场心魔 +20% 核心/路障攻击力；每 5 秒在 Boss 当前位置召唤 1 只焦虑·疾走者。
- 非焦虑 Boss：展开本类型压迫技能，全场心魔获得护盾；若进入 `confront` 狂暴状态，额外获得减伤；每 5 秒召唤与 Boss 本体类型一致的心魔，例如强迫 Boss 召唤强迫，抑郁 Boss 召唤抑郁，创伤 Boss 召唤创伤。
...
| 边界桩   | 路线肉盾    | 0           | 路线格 | 耐久 150 | 60  | 只能种在路线，阻挡心魔并压制创伤闪回       |


当前 6 种塔对应 6 个明确策划职能：美好回忆塔处理密集低血心魔；信念塔处理高血单体；共鸣塔负责减速、反隐和多目标控制；自我接纳塔把防守压力转化为 SAN 回复；洞察塔按当前生命百分比造成伤害，用来反制后期厚血和 Boss；边界桩只能部署在路线格上，用耐久换取短时间阻挡，并在接触创伤时压制其受击闪回位移。
...
| 创伤·闪回  | 70 / 36 / 10 | flicker（受伤瞬间向前闪现，接触边界桩时被压制） | 需要边界桩控位后集火        |
...
地图采用格子制，路线和塔位按 `level_id` 从 CSV 读取。第 1 关保留教学图；第 2-6 关分别配置独立路线、建造格、入口节奏和地图机制。每关都保留 `short / long / edge` 三类路线定义，但每波实际开放路线由关卡规则、当前波配置和复盘策略共同约束，不强制每波都三线同时开放。

新增关卡地图差异：

| 关卡 | 地图结构 | 强机制 |
| --- | --- | --- |
| 失眠首夜 | 教学图，路线逐步开放 | 教学塔、心魔、残堆、Boss 谈判 |
| 呼吸错拍 | 沙漏形双肺路线 | `breath_vent` 每 6 秒切换吸气 / 呼气，影响速度与受伤倍率 |
| 镜室回声 | 上下对称镜室 | `mirror_gate` 延迟生成回声体，摧毁镜门可关闭本波回声 |
| 枯井沉降 | 稀疏岛状塔位 | `dry_well` 存活时压制周围塔位，摧毁后返还大量念力、释放塔位，并在后续波次保持清除 |
| 边界裂隙 | 三入口地图，边路提前开放 | `fracture_node` 强化边路加速，边界桩可压制裂隙半径内效果 |
| 整合审判 | 三线汇入中央审判区 | `trial_obelisk` 为精英提供护盾、嘲讽优先级和减伤 |
...
当前版本共有 6 个关卡，每关 10 波。第 1 关是完整教学关；第 2-6 关默认基础塔、心魔和基础技能已解锁，前 2-3 波只教学本关特殊机制，第 5 波前形成第一次明确考验，第 10 波是终局压力波。

| 关卡 | 名称 | 特殊规则 | 设计目标 |
| --- | --- | --- | --- |
| 1 | 失眠首夜 | `tutorial` | 完成基础教学：放塔、升级拆除、路线变化、残堆、Boss 谈判 |
| 2 | 呼吸错拍 | `breath_phase` | 前半波快冲、后半波慢厚，要求玩家切换 AOE、单体与阻挡节奏 |
| 3 | 镜室回声 | `echo_group` | 主攻组延迟回声，考验拆镜门和分区火力承接 |
| 4 | 枯井沉降 | `scarcity` | 念力紧缺、塔位稀疏，强调拆资源、扩塔位和升级续航 |
| 5 | 边界裂隙 | `fracture_edge` | 边路从早期开放，要求边界桩和交汇处防守参与核心解法 |
| 6 | 整合审判 | `trial_elite` | 精英、护盾、嘲讽、破隐、AOE、单体和阻挡的最终综合考 |
...
> **关键设计点**：玩家在「心魔复盘」面板看到的 `next_strategy` 不是装饰文本，而是**真的会被翻译成下一波 spawn 表的代码改动**。复盘面板还会列出"已生效的下波变化"。当前波的可用路线先由 `level_id` 对应地图和路线规则限定，`path_weight_shift` 只在可用路线内改变抽样倾向；阵型重排发生在阵容变化之后，确保前排、嘲讽、护盾单位能按设计先进入火力区。
...
每波开场，根据 `waveIndex` 与 `emotionHint` 生成 80~120 字第三人称情境 vignette，建立情感代入。
...
- **关卡地图投影**：`Grid.createMapProjection(level_id)` 生成当前关卡、当前波的开放路线、阻塞格、可建造格和强机制地图物；玩家塑形新增的塔位会作为 `extraBuildCells` 参与下一波投影；不再有效的念力残堆会随地图变化清理。
...
| 多关卡地图与机制       | D17-18 | 新增 5 关完整 10 波内容，按 `level_id` 拆分路线、塔位、路线规则和地图元素；运行时配置扩展到 17 张 CSV |
